...
class Signup extends React.Component {
    ...
    handleSaveDatas = async () => {
        if (this.state.title === '' || this.state.activity === '')
            ToastAndroid.show('Por favor, verifique se todos os campos estão preenchidos.', ToastAndroid.LONG)
        else {
    ...
    render() {
        return (
            <View style={signupStyle.main}>
                <Text style={signupStyle.title}>{'CADASTRO'}</Text>
                ...
                <View style={signupStyle.inputActivity}>
                    <TextInput
                        ref={(input) => { this.textActivity = input }}
